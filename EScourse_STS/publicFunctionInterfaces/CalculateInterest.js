...
function CalculateInterest()
{
    ...
    {
        let calculateIntSqlOne = "select * from capitalaccount where capitalaccountid = ?";
        let params = [capitalaccountid];
        //获取本次更新前账户内余额
        var tempmoney;
        var tempinterest;
        var lastupdate_y;
        ...
        var thisupdate_d;
        var intervaltime;
        dbQuery(calculateIntSqlOne, params, function (err, result) {
            if (err) {
            ...
            tempmoney = result[0].availablemoney;
                //获取上次更新时间
                let calculateIntSqlTwo = "select iotime,year(iotime) as lastupdate_y,month(iotime) as lastupdate_m,day(iotime) as lastupdate_d from capitalaccountio where capitalaccountid = ? order by iotime desc";
                let params1 = [capitalaccountid];
                dbQuery(calculateIntSqlTwo, params1, function (err,result) {
                    if (err) {
                        console.log('[SELECT ERROR] - ', err.message);
                        return ;
                    }
                        ...
                        dbQuery(calculateIntSqlThree, params2, function (err,result) {
                            if (err) {
                                console.log('[SELECT ERROR] - ', err.message);
                                return;
                            }
