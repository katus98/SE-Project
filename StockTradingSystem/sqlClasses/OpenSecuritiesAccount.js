...
function OpenAccount() {
    ...
    this.insertpaccount=function(data,req,callback)
    {
        ...
        dbQuery(getSql0,[],function(err,result){
            ...
            else
            {
                ...
                {
                    //表中已有同样身份信息的记录，查看这个记录的状态是否为fozen
                    if(result[0].s=='frozen'){callback(-1);}
                    if(result[0].s=='normal'){callback(-2);}
                }
                else
                {
                    //给idreference表插入数据
    ...
    this.initstockhold=function(personid,callback){
         let getSql0="select code from stock";
         dbQuery(getSql0,[],function(err,result0){
            if (err) {
                console.log('[insert  STOCKHOLD ERROR] - ', err.message);
                return;
            }
            for(let i=0;i<result0.length;i++)
            {
                let getSql1="insert into stockhold(personid,stockid,stocknum,stockcost) values(?,?,0,0)";
                let SqlParams1=[personid,result0[i].code];
                dbQuery(getSql1,SqlParams1,function(err,result){
                        if (err) {
